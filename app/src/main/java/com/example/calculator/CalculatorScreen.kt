// CalculatorScreen.kt 파일: 메인 계산기 화면(Compose UI) + 다이얼로그(추가/수정) UI를 정의함

package com.example.stockcalculator.com.example.calculator // 이 Kotlin 파일이 속한 패키지(네임스페이스). 다른 파일과 패키지가 맞아야 서로 참조가 쉬움.
...
fun CalculatorScreen( // 메인 계산기 화면을 그리는 Composable 함수 시작.
    ...
    Scaffold( // Material3의 기본 화면 레이아웃(상단바/본문/FAB 등)을 제공하는 컨테이너.
        ...
        editingStock?.let { s -> // editingStock이 null이 아니면(=수정할 종목이 선택되어 있으면) 수정 다이얼로그 표시.
            ...
            ) // EditStockDialog 호출 끝.
        } // let 끝.
    } // Scaffold content 끝.
} // CalculatorScreen 끝.

@Composable // 이 함수도 UI를 그리는 Composable.
fun StockItem(stock: Stock, totalAssets: Double, onDelete: () -> Unit, onEdit: () -> Unit) { // 종목 1개를 카드 형태로 표시하고 수정/삭제 콜백을 받음.
    ...
    Card(elevation = CardDefaults.cardElevation(2.dp)) { // 각 종목을 카드로 감싸 시각적으로 구분(그림자 높이 2dp).
        Column(modifier = Modifier.padding(16.dp)) { // 카드 내부는 세로 정렬, 내부 여백 16dp.
...
@Composable // “추가” 다이얼로그도 Composable.
...
@Composable // “수정” 다이얼로그도 Composable.
fun EditStockDialog( // 기존 종목을 수정하기 위한 다이얼로그 함수 시작.
    ...
    // ⚠️ 참고: 여러 종목을 연속으로 수정할 때 초기값이 꼬이는 걸 막으려면 remember(stock.id)로 키를 주는 게 더 안전함.

    AlertDialog( // 수정 다이얼로그 UI 시작.
