// CalculatorViewModel.kt 파일: 화면(UI)에서 쓰는 “데이터/상태”를 보관하고,
// 추가/수정/삭제 같은 비즈니스 로직을 처리한 뒤 저장소(StockStorage)에 반영하는 ViewModel

package com.example.stockcalculator.com.example.calculator // 이 ViewModel이 속한 패키지(네임스페이스). UI/모델/저장소 파일들과 패키지가 맞아야 참조가 쉬움.
...
class CalculatorViewModel(application: Application) : AndroidViewModel(application) { // Application 컨텍스트가 필요한 로직(저장소)을 위해 AndroidViewModel 사용.
    ...
    init { // ViewModel이 생성되는 시점에 1번 실행되는 초기화 블록.
        loadData() // 앱에 저장돼 있던 데이터를 불러와 StateFlow들에 반영.
    }

    private fun loadData() { // 저장소에서 모든 데이터를 읽어와 ViewModel 상태를 채우는 함수.
    ...
    private fun calculateTotalAssets() { // 종목들의 현재 평가액(currentValue) 합으로 총자산을 계산하는 함수.
    ...
    private fun recordAssetHistory() { // “현재 시점의 총자산”을 히스토리에 저장하는 함수(스냅샷 기록).
    ...
    fun addStock(name: String, targetRatio: Double, currentValue: Double) { // 종목을 새로 추가하는 공개 함수(UI에서 호출).
    ...
    fun updateStock(stockId: String, name: String, targetRatio: Double, currentValue: Double) { // 특정 종목(id)을 수정하는 공개 함수.
    ...
    fun deleteStock(stockId: String) { // 특정 종목(id)을 삭제하는 공개 함수.
    ...
    fun loadPreset(preset: PortfolioPreset) { // 선택한 프리셋을 현재 종목 리스트로 “적용”하는 함수.
    ...
    fun addPreset(name: String, description: String, selectedStocks: List<Stock>) { // 프리셋을 새로 저장하는 함수.
    ...
    fun updatePreset(presetId: String, newName: String, newStocks: List<Stock>) { // 기존 프리셋의 이름/종목 구성을 수정하는 함수.
    ...
    fun deletePreset(presetId: String) { // 프리셋 삭제 함수.
    ...
    fun addEvent(title: String, date: LocalDate) { // 캘린더 이벤트 추가 함수.
    ...
    fun deleteEvent(event: CalendarEvent) { // 캘린더 이벤트 삭제 함수(객체를 받아 id로 제거).
